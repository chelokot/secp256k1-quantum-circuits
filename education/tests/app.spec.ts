...
test('runs the qubit and netlist interactives', async ({ page }) => {
  ...
  await expect(page.getByTestId('one-qubit-patterns-lab')).toContainText('Hadamard mix');
  await expect(page.getByTestId('one-qubit-patterns-lab')).toContainText('bit flip swap');
...
test('lets the learner derive peak qubits from a mini engine', async ({ page }) => {
  ...
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('Row witness inspector');
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('row 4 witness');
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('10 live qubits');
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('partial scratch 2q -> lookup_workspace');
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('lookup_workspace: 5/3');
  ...
  await page.getByRole('button', { name: 'jump to peak row' }).click();
  ...
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('row 2 witness');
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('9 live qubits');
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('partial scratch 2q -> scratch_workspace');
  await expect(page.getByTestId('mini-resource-engine-lab')).toContainText('scratch_workspace: 2/2');
