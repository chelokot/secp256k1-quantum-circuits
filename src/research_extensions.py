...
def compute_dominant_cost_breakdown(repo_root: Path) -> Dict[str, Any]:
    ...
    out_path = repo_root / "artifacts" / "optimized" / "out" / "dominant_cost_breakdown.json"
    dump_json(out_path, result)
    return result


def compute_literature_projection_scenarios(repo_root: Path) -> Dict[str, Any]:
    breakdown = load_json(repo_root / "artifacts" / "optimized" / "out" / "dominant_cost_breakdown.json")
    projection = load_json(repo_root / "artifacts" / "optimized" / "out" / "resource_projection.json")
    folded = load_json(repo_root / "artifacts" / "optimized" / "out" / "lookup_folded_projection.json")
    ...
    out_path = repo_root / "artifacts" / "optimized" / "out" / "literature_projection_scenarios.json"
    dump_json(out_path, result)
    return result
...
def run_challenge_ladder_audit(repo_root: Path, max_random_scalars_per_curve: int = 128) -> Dict[str, Any]:
    ...
    family = load_json(repo_root / "artifacts" / "optimized" / "out" / "optimized_pointadd_family.json")
    out_dir = repo_root / "benchmarks" / "challenge_ladder"
...
def run_research_pass(repo_root: Path) -> Dict[str, Any]:
    ...
    result = {
        ...
        "lookup_folding": {
            "contract_sha256": sha256_path(repo_root / "artifacts" / "optimized" / "out" / "lookup_signed_fold_contract.json"),
            "folded_scaffold_sha256": sha256_path(repo_root / "artifacts" / "optimized" / "out" / "ecdlp_scaffold_lookup_folded.json"),
