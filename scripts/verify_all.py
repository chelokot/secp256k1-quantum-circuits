...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Run the quick repository verification path.')
...
def build_summary(console: Console, show_progress: bool) -> Dict[str, Any]:
...
def print_human_summary(summary: Dict[str, Any], console: Console) -> None:
    ...
    print(f"[1/2] Deterministic secp256k1 audit  {console.ok('PASS') if checks['optimized_audit_pass'] else console.fail('FAIL')}")
    ...
    print(f"[2/2] Toy-curve finite-model check   {console.ok('PASS') if toy_pass else console.fail('FAIL')}")
    ...
    print(f"  lower modeled non-Clifford cost vs Google low-qubit line: {projection['improvement_vs_google']['versus_low_qubit']['toffoli_gain_2lookup']:.4f}x (2-channel), {projection['improvement_vs_google']['versus_low_qubit']['toffoli_gain_3lookup']:.4f}x (3-channel)")
    print(f"  lower modeled non-Clifford cost vs Google low-gate line:  {projection['improvement_vs_google']['versus_low_gate']['toffoli_gain_2lookup']:.4f}x (2-channel), {projection['improvement_vs_google']['versus_low_gate']['toffoli_gain_3lookup']:.4f}x (3-channel)")


def main() -> None:
    args = parse_args()
    console = Console(color=not args.no_color and sys.stdout.isatty())
    summary = build_summary(console, show_progress=not args.json)
    if args.json:
        print(json.dumps(summary, indent=2))
        return

    print_human_summary(summary, console)
