...
- `public_engine_manifest.json` — no-ZKP public engine manifest deriving public totals from the public-candidate flat netlist plus materialized liveness peak, then binding executable instruction/wire/schedule/owner/resource-term streams, the flat execution probe, semantic-boundary evidence, and primitive-operation evidence from arithmetic operation IR, QROAM primitive certificate, and phase-shell lowering
