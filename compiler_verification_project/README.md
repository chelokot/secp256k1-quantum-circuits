...
- `public_engine_manifest.json` — no-ZKP public engine manifest binding executable instruction/wire/schedule/owner/resource-term streams, semantic-boundary evidence, and primitive-operation evidence from arithmetic operation IR, QROAM primitive certificate, and phase-shell lowering
...
artifact: `fast_engine_verify.py` regenerates it, checks its semantic evidence,
and rejects arithmetic-operation, QROAM-cost/workspace, or phase-shell drift
before compressed or Groth16 proof work is relevant.
