...
MODULAR_ACCUMULATOR_CAPACITY_CERTIFICATE_SCHEMA = 'compiler-project-modular-accumulator-capacity-certificate-v1'
...
def build_modular_accumulator_capacity_certificate(
    *,
    ...
    product_column_capacity_bits = 2 * field_bits - 1
    ...
    owner_capacity_obligations = [
        {
            ...
            'logical_qubit_budget_required_by_materialized_columns': product_column_capacity_bits,
    ...
    checks = {
        ...
        'product_column_owner_exceeds_single_field_slot': (
            product_column_capacity_bits == int(row_stream['max_source_column']) + 1
            and product_column_capacity_bits > field_bits
        ),
    ...
    return {
        ...
        'definition': 'Generated owner-capacity certificate for the modular accumulator row-stream obligations. It distinguishes materialized obligation-order capacity from unpromoted serialized candidates so temporary, product-column, guard, and fold-overflow wires cannot be treated as free public resources.',
        'source_digests': {
        ...
        'promotion_status': {
            ...
            'required_to_promote': [
                ...
                'Lower product-column accumulation so the 511-column owner is either streamed with proven non-concurrency or counted explicitly.',
