...
def load_compiler_artifacts(repo_root: Path) -> Dict[str, Any]:
    ...
    if not all(path.exists() for path in required.values()):
        ...
        dump_json(
            artifact_root / 'modular_accumulator_full_adder_stream.json',
        ...
        dump_json(
            artifact_root / 'modular_accumulator_full_adder_reversibility.json',
...
def build_modular_primitive_wire_audit_checks(artifacts: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    checks = [
        ...
        _check('modular_primitive_wire_audit_records_current_wire_gaps', audit['pass'] is False and audit['checks']['raw_field_operand_liveness_is_not_claimed_complete'] is True and audit['checks']['field_operand_wires_are_live_or_classified'] is True and audit['checks']['no_blocking_field_liveness_gaps'] is True and audit['checks']['lookup_virtual_field_operands_are_classified'] is True and audit['checks']['no_unresolved_virtual_field_operands'] is True and audit['checks']['synthetic_scratch_wires_are_single_use_ccx_targets'] is True and audit['checks']['synthetic_scratch_wires_have_cleanup_or_counted_capacity'] is False and audit['checks']['no_synthetic_arithmetic_scratch_wires_without_owner_capacity'] is False and audit['field_wire_missing_liveness_count'] == audit['field_wire_classified_missing_liveness_count'] and audit['field_wire_blocking_missing_liveness_count'] == 0 and audit['overwritten_source_field_observation_count'] > 0 and audit['unresolved_virtual_field_observation_count'] == 0 and audit['arithmetic_scratch_wire_observation_count'] > 0, 'current modular primitive stream has classified virtual field operands and no blocking field liveness gap; single-use synthetic arithmetic scratch targets remain abandoned', {'pass': audit['pass'], 'checks': audit['checks'], 'field_missing': audit['field_wire_missing_liveness_count'], 'field_classified_missing': audit['field_wire_classified_missing_liveness_count'], 'field_blocking_missing': audit['field_wire_blocking_missing_liveness_count'], 'overwritten_source_fields': audit['overwritten_source_field_observation_count'], 'lookup_virtual_fields': audit['lookup_virtual_field_observation_count'], 'unresolved_virtual_fields': audit['unresolved_virtual_field_observation_count'], 'scratch_observations': audit['arithmetic_scratch_wire_observation_count'], 'scratch_single_use': audit['arithmetic_scratch_single_use_wire_count'], 'scratch_abandoned_garbage': audit['arithmetic_scratch_abandoned_garbage_count']}),
    ]
    return _summarize_checks(checks)
...
def build_modular_accumulator_capacity_certificate_checks(artifacts: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    checks = [
        ...
        _check('modular_accumulator_capacity_certificate_rejects_free_product_columns', obligations['streamed_product_accumulator_column_space']['partial_product_column_count'] == 2 * FIELD_BITS - 1 and obligations['streamed_product_accumulator_column_space']['logical_qubit_budget_required_by_materialized_columns'] == 2 * FIELD_BITS and obligations['streamed_product_accumulator_column_space']['fits_single_field_slot'] is False, '511 product columns plus final carry bit cannot be hidden inside one 256-bit field slot', obligations['streamed_product_accumulator_column_space']),
...
def build_modular_accumulator_semantic_obligations_checks(artifacts: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    checks = [
        ...
        _check('modular_accumulator_semantic_obligations_keep_resource_shortcuts_rejected', classes['partial_product_column_consume']['required_logical_qubit_capacity'] == 2 * FIELD_BITS and classes['partial_product_column_consume']['single_field_slot_shortcut_allowed'] is False and classes['pseudo_mersenne_high_column_fold']['overflowing_shift_column_count'] == 31 and classes['pseudo_mersenne_high_column_fold']['fits_single_field_slot_without_second_fold'] is False, 'carry-complete product owner and high-fold overflow cannot be hidden in one field slot', classes),
...
def build_modular_accumulator_full_adder_stream_checks(artifacts: Mapping[str, Any]) -> Dict[str, Any]:
    stream = artifacts['modular_accumulator_full_adder_stream']
    expected = build_modular_accumulator_full_adder_stream(
        modular_accumulator_carry_save_candidate=artifacts['modular_accumulator_carry_save_candidate'],
        modular_accumulator_full_adder_contract=artifacts['modular_accumulator_full_adder_contract'],
    )
    contract_totals = artifacts['modular_accumulator_full_adder_contract']['candidate_totals']
    checks = [
        _check('modular_accumulator_full_adder_stream_matches_generator', stream == expected, expected, stream),
        _check('modular_accumulator_full_adder_stream_schema_is_current', stream['schema'] == MODULAR_ACCUMULATOR_FULL_ADDER_STREAM_SCHEMA, MODULAR_ACCUMULATOR_FULL_ADDER_STREAM_SCHEMA, stream['schema']),
        _check('modular_accumulator_full_adder_stream_passes_internal_checks', stream['pass'] is True and all(stream['checks'].values()), True, stream['checks']),
        _check('modular_accumulator_full_adder_stream_counts_match_contract', stream['full_adder_cell_count'] == contract_totals['full_adder_cell_count'] and stream['primitive_counts_total'] == contract_totals['embedded_full_adder_primitive_counts'], 'streamed cells and gates derive from full-adder contract', stream),
        _check('modular_accumulator_full_adder_stream_retained_inputs_are_explicit', stream['retained_input_obligation_bits'] == contract_totals['retained_input_obligation_bits'] and stream['output_obligation_bits'] == contract_totals['output_obligation_bits'], 'retained inputs and outputs remain resource obligations', stream),
        _check('modular_accumulator_full_adder_stream_has_segmented_primitive_hash', len(stream['operation_stream_sha256']) == 64 and stream['segment_count'] == len(stream['segments']) and stream['operation_count'] > 0, 'stream hash and segments materialize every emitted primitive row', stream),
        _check('modular_accumulator_full_adder_stream_remains_unpromoted', stream['promotion_status']['status'] == FULL_ADDER_STREAM_UNPROMOTED_STATUS, 'full-adder stream is exact but not globally promoted', stream['promotion_status']),
    ]
    return _summarize_checks(checks)
...
def build_modular_accumulator_full_adder_reversibility_checks(artifacts: Mapping[str, Any]) -> Dict[str, Any]:
    reversibility = artifacts['modular_accumulator_full_adder_reversibility']
    expected = build_modular_accumulator_full_adder_reversibility(
        modular_accumulator_full_adder_liveness=artifacts['modular_accumulator_full_adder_liveness'],
    )
    checks = [
        _check('modular_accumulator_full_adder_reversibility_matches_generator', reversibility == expected, expected, reversibility),
        _check('modular_accumulator_full_adder_reversibility_schema_is_current', reversibility['schema'] == MODULAR_ACCUMULATOR_FULL_ADDER_REVERSIBILITY_SCHEMA, MODULAR_ACCUMULATOR_FULL_ADDER_REVERSIBILITY_SCHEMA, reversibility['schema']),
        _check('modular_accumulator_full_adder_reversibility_passes_internal_checks', reversibility['pass'] is True and all(reversibility['checks'].values()), True, reversibility['checks']),
        _check('modular_accumulator_full_adder_reversibility_finds_three_input_class', reversibility['max_preimage_size'] == 3 and reversibility['minimum_witness_bits_per_cell'] == 2, 'full-adder sum/carry output has a size-three preimage class', reversibility),
        _check('modular_accumulator_full_adder_reversibility_rejects_one_bit_witness', reversibility['one_bit_witness_exhaustive_search']['candidate_function_count'] == 256 and reversibility['one_bit_witness_exhaustive_search']['collision_free_function_count'] == 0, 'all one-bit witness functions collide', reversibility['one_bit_witness_exhaustive_search']),
        _check('modular_accumulator_full_adder_reversibility_witness_total_matches_liveness', reversibility['minimum_witness_bits_total'] == 2 * artifacts['modular_accumulator_full_adder_liveness']['full_adder_cell_count'], 'two witness bits per cell are required unless source uncompute is proven', reversibility),
        _check('modular_accumulator_full_adder_reversibility_remains_unpromoted', reversibility['promotion_status']['status'] == FULL_ADDER_REVERSIBILITY_UNPROMOTED_STATUS, 'full-adder reversibility lower bound is exact but not globally promoted', reversibility['promotion_status']),
    ]
    return _summarize_checks(checks)
...
def build_proof_publication_status_checks(artifacts: Mapping[str, Any], repo_root: Path) -> Dict[str, Any]:
    ...
    checks = [
        ...
        _check('proof_publication_status_ready_matches_proof_status_and_public_headline', status['publication_ready'] == (proof_status['all_current'] and artifacts['public_headline_result']['pass'] is True), {'proof_status_all_current': proof_status['all_current'], 'public_headline_pass': artifacts['public_headline_result']['pass']}, status['publication_ready']),
...
def build_public_headline_result_checks(artifacts: Mapping[str, Any], repo_root: Path) -> Dict[str, Any]:
    ...
    checks = [
        ...
        _check('public_headline_result_blocks_publication_until_clifford_complete', public_result['pass'] is False and public_result['checks']['full_clifford_complete_engine_ready_for_publication'] is False and any(blocker['active'] is True and blocker['name'] == 'remaining_macro_boundaries_not_flattened' for blocker in public_result['publication_blockers']), 'active macro-boundary publication blocker', {'pass': public_result['pass'], 'full_clifford_complete_engine_ready_for_publication': public_result['checks']['full_clifford_complete_engine_ready_for_publication'], 'publication_blockers': public_result['publication_blockers']}),
