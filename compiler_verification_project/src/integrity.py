...
def build_modular_primitive_wire_audit_checks(artifacts: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    checks = [
        ...
        _check('modular_primitive_wire_audit_records_current_wire_gaps', audit['pass'] is False and audit['checks']['field_operand_wires_are_live_in_trace'] is False and audit['checks']['lookup_virtual_field_operands_are_classified'] is True and audit['checks']['no_unresolved_virtual_field_operands'] is True and audit['checks']['synthetic_scratch_wires_are_single_use_ccx_targets'] is True and audit['checks']['synthetic_scratch_wires_have_cleanup_or_counted_capacity'] is False and audit['checks']['no_synthetic_arithmetic_scratch_wires_without_owner_capacity'] is False and audit['overwritten_source_field_observation_count'] > 0 and audit['unresolved_virtual_field_observation_count'] == 0 and audit['arithmetic_scratch_wire_observation_count'] > 0, 'current modular primitive stream has no unresolved virtual field operands; single-use synthetic arithmetic scratch targets remain abandoned', {'pass': audit['pass'], 'checks': audit['checks'], 'overwritten_source_fields': audit['overwritten_source_field_observation_count'], 'lookup_virtual_fields': audit['lookup_virtual_field_observation_count'], 'unresolved_virtual_fields': audit['unresolved_virtual_field_observation_count'], 'scratch_observations': audit['arithmetic_scratch_wire_observation_count'], 'scratch_single_use': audit['arithmetic_scratch_single_use_wire_count'], 'scratch_abandoned_garbage': audit['arithmetic_scratch_abandoned_garbage_count']}),
    ]
    return _summarize_checks(checks)
