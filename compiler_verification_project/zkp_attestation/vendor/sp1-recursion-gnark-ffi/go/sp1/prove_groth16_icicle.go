...
var globalR1cs constraint.ConstraintSystem = icicle_groth16.NewCS(ecc.BN254)
var globalR1csInitialized = false
var globalPk groth16.ProvingKey = icicle_groth16.NewProvingKey(ecc.BN254)
...
func ProveGroth16(dataDir string, witnessPath string) Proof {
	...
	if !globalR1csInitialized {
		...
		normalizeGroth16Commitments(globalR1cs)
	...
	start = time.Now()
	// Generate the proof.
	proof, err := icicle_groth16.Prove(globalR1cs, globalPk, witness)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		panic(err)
	}
