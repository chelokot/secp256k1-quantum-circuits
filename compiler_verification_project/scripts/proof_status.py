...
PROOF_MANIFEST_PATH = PROJECT_ROOT / 'artifacts' / 'package' / 'proof_manifest.json'
...
def proof_manifest_files() -> dict[str, Any]:
    manifest = load_json(PROOF_MANIFEST_PATH)
    return manifest['files']


def manifest_file_status(relative_path: str | None, manifest_files: dict[str, Any]) -> dict[str, Any]:
...
def proof_file_status(fixture: dict[str, Any], manifest_files: dict[str, Any]) -> dict[str, Any]:
    ...
    observed_sha256 = sha256_path(path) if exists else None
    observed_size = path.stat().st_size if exists else None
    manifest_status = manifest_file_status(proof_path, manifest_files)
    return {
...
def verifier_key_status(fixture: dict[str, Any], manifest_files: dict[str, Any]) -> dict[str, Any]:
    ...
    observed_sha256 = sha256_path(path) if exists else None
    observed_size = path.stat().st_size if exists else None
    manifest_status = manifest_file_status(verifier_key_path, manifest_files)
    return {
...
def fixture_status(system: str, input_payload: dict[str, Any], public_values: dict[str, Any], manifest_files: dict[str, Any]) -> dict[str, Any]:
    fixture_relative_path = f'compiler_verification_project/artifacts/zkp_attestation_reusable_chunk_candidate/zkp_attestation_fixture_{system}.json'
    fixture = load_json(PROJECT_ROOT / fixture_relative_path)
    proof_status = proof_file_status(fixture, manifest_files)
    key_status = verifier_key_status(fixture, manifest_files)
    fixture_manifest_status = manifest_file_status(fixture_relative_path, manifest_files)
...
def build_report() -> dict[str, Any]:
    ...
    manifest_files = proof_manifest_files()
    systems = {
        system: fixture_status(system, input_payload, public_values, manifest_files)
    ...
    return {
        ...
        'proof_manifest': 'artifacts/package/proof_manifest.json',
        'proof_manifest_sha256': sha256_path(PROOF_MANIFEST_PATH),
