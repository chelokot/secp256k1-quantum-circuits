...
`artifacts/out/optimized_pointadd_secp256k1.json` is the primary
...
`artifacts/out/ecdlp_scaffold_optimized.json` is a machine-readable
...
`artifacts/out/resource_projection.json` and
`artifacts/out/lookup_folded_projection.json` are explicit backend
