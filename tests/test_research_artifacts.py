...
class ResearchArtifactTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dominant = json.loads((REPO_ROOT / 'artifacts' / 'optimized' / 'out' / 'dominant_cost_breakdown.json').read_text())
        cls.scenarios = json.loads((REPO_ROOT / 'artifacts' / 'optimized' / 'out' / 'literature_projection_scenarios.json').read_text())
        cls.lookup_summary = json.loads((REPO_ROOT / 'artifacts' / 'optimized' / 'out' / 'lookup_signed_fold_summary.json').read_text())
        cls.lookup_projection = json.loads((REPO_ROOT / 'artifacts' / 'optimized' / 'out' / 'lookup_folded_projection.json').read_text())
    ...
    def test_cost_model_correction_is_reflected(self):
        self.assertGreater(self.dominant['breakdown']['arithmetic_share_fraction_2lookup'], 0.88 - 1e-9)
        self.assertGreater(self.dominant['breakdown']['arithmetic_share_fraction_3lookup'], 0.83 - 1e-9)
        self.assertLess(self.dominant['breakdown']['lookup_share_fraction_2lookup'], 0.13)
        self.assertLess(self.dominant['breakdown']['lookup_share_fraction_3lookup'], 0.18)
        self.assertGreater(self.dominant['ceilings']['max_total_reduction_fraction_from_arithmetic_only_2lookup'], 0.88 - 1e-9)
        self.assertGreater(self.dominant['ceilings']['max_total_reduction_fraction_from_arithmetic_only_3lookup'], 0.83 - 1e-9)
    ...
    def test_lookup_folding_projection_is_meaningful(self):
        ...
        self.assertGreater(base['improvement_fraction_vs_current_2channel'], 0.05)
        self.assertGreater(base['improvement_fraction_vs_current_3channel_conservative'], 0.08)
